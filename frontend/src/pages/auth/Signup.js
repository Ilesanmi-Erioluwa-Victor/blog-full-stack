...
const Signup = () => {
  ...
  const handleInputSubmit = async (event) => {
    ...
    try {
      ...
      await dispatch(userRegisterAction(userInfo));
 
      setTimeout(() => {
        if (userInfo?.registered?.firstName) {
          navigate(`/verify-account/student_verification/${email}`);
        } else {
          navigate(`/verify-account/tutor_verification/${email}`);
        }
      }, 500);
       ...
       <div className='flex padding bg w[100%] relative gap-4 items-center'>
         ...
         <section className='w-[40%] sec-flex none'>
           ...
           <p className='mt-[-.5rem]'>
             ...
             beatae sed illum error quo dicta in exercitationem aperiam
             doloribus neque?
           </p>
